...
/**
 * Generate safe HTML tag and link attributes from Elementor URL control.
 *
 * @param array  $link     Elementor URL control settings.
 * @param string $html_tag HTML tag (div, span, a).
 * @return array
 */
function xpro_elementor_get_link_attributes( $link, $html_tag ) {

	$allowed_tags = array( 'div', 'span', 'a', 'p', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6' );
	...
	// Convert to anchor if URL exists.
	if ( ! empty( $link['url'] ) ) {
		$data['tag'] = 'a';
		$data['attr'] .= ' href="' . esc_url( $link['url'] ) . '"';
	}
	...
	if ( ! empty( $link['custom_attributes'] ) ) {
		...
		foreach ( $attributes as $attribute ) {
			...
			$attr_name  = trim( $custom_attr[0] ?? '' );
			$attr_value = trim( $custom_attr[1] ?? '' );

			// Prevent JavaScript/event attributes.
			if (
				preg_match( '/^on/i', $attr_name ) ||
				stripos( $attr_name, 'javascript' ) !== false ||
				stripos( $attr_value, 'javascript:' ) !== false
			) {
				continue;
			}
