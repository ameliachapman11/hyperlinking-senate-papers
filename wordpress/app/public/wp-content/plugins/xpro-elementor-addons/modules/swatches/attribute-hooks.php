...
class Attribute_Hooks {
	...
	public function init() {
		...
		foreach ( $attribute_taxonomies as $tax ) {
			...
			if ( isset( $types[ $tax->attribute_type ] ) ) {
				...
				add_filter( 'manage_edit-pa_' . $tax->attribute_name . '_columns', array( $this, $col ) );
				add_filter( 'manage_pa_' . $tax->attribute_name . '_custom_column', array( $this, 'add_attr_column_content' ), 10, 3 );

				add_action( 'pa_' . $tax->attribute_name . '_add_form_fields', array( $this, 'add_attr_field' ) );
				add_action( 'pa_' . $tax->attribute_name . '_edit_form_fields', array( $this, 'edit_attr_field' ), 10, 2 );
			}
		}

		add_action( 'created_term', array( $this, 'persist_term_meta' ), 10, 2 );
		add_action( 'edit_term', array( $this, 'persist_term_meta' ), 10, 2 );
		add_action( 'xpro_attribute_field_chain', array( $this, 'attribute_fields' ), 10, 3 );
	}


	public function add_attr_field( $taxonomy ) {

		$attr = Swatches::get_tax_attribute( $taxonomy );

		do_action( 'xpro_attribute_field_chain', $attr->attribute_type, '', 'add' );
	}
	...
	public function edit_attr_field( $term, $taxonomy ) {
		...
		do_action( 'xpro_attribute_field_chain', $attr->attribute_type, $value, 'edit' );
	}

	public function label_attribute_column( $columns ) {
		$th = esc_html__( 'Label', 'xpro-elementor-addons' );
		return $this->add_attr_column( $columns, $th );
	}

	public function image_attribute_column( $columns ) {
		$th = esc_html__( 'Thumbnail', 'xpro-elementor-addons' );
		return $this->add_attr_column( $columns, $th );
	}

	public function color_attribute_column( $columns ) {
		$th = esc_html__( 'Color', 'xpro-elementor-addons' );
		return $this->add_attr_column( $columns, $th );
	}
	...
	public function add_attr_column_content( $columns, $column, $term_id ) {
		...
		$attr = Swatches::get_tax_attribute( sanitize_key( $_REQUEST['taxonomy'] ) );
		...
		switch ( $type ) {
			case Swatches::PA_IMAGE:
				...
				<div style="line-height:60px;">
					<input type="hidden" class="xpro_term_img" name="<?php echo esc_attr( $type ); ?>"
						   value="<?php echo esc_attr( $value ); ?>"/>

					<button type="button" class="xpro_upload_img_button button">
						<?php esc_html_e( 'Upload image', 'xpro-elementor-addons' ); ?>
					</button>

					<button type="button"
							class="xpro_remove_img_btn button <?php echo esc_attr( $value ) ? '' : 'hidden'; ?>">
