...
$link_url        = '';
$link_target     = '';
$author_name_tag = sanitize_key( $settings['author_name_tag'] );
...
if ( 'current' === $settings['source'] ) {
	...
	$author['avatar']       = get_avatar_url( $user_id, [ 'size' => 300 ] );
	...
	if ( ! empty( $settings['author_avatar']['url'] ) ) {
		...
		if ( ! empty( $settings['author_avatar']['id'] ) ) {
			$img = wp_get_attachment_image_src( $settings['author_avatar']['id'], 'medium' );
			if ( ! empty( $img[0] ) ) {
				$avatar_src = $img[0];
			}
		}
	...
	$author['posts_url']    = ( 'yes' === $settings['show_link'] )
		? ( $settings['posts_url']['url'] ?? '' )
		: '';
}
...
$print_link   = (
	( ! $custom_src && 'yes' === $settings['show_link'] && ! empty( $settings['link_text'] ) )
	||
	( $custom_src && ! empty( $author['posts_url'] ) && ! empty( $settings['link_text'] ) )
);
...
if ( ( ! empty( $settings['link_to'] ) || $custom_src ) ) {
	...
	if ( ( $custom_src || 'website' === $settings['link_to'] ) && ! empty( $author['website'] ) ) {
		...
		$this->add_render_attribute( 'author_link', 'href', esc_url( $author['website'] ) );
		$this->add_render_attribute( 'author_link', 'target', '_blank' );

		if ( ! empty( $settings['author_website']['custom_attributes'] ) ) {
			...
			foreach ( $attributes as $attribute ) {
				...
				if (
					in_array( $name, [ 'title', 'class', 'id', 'rel', 'target' ], true ) ||
					strpos( $name, 'data-' ) === 0 ||
					strpos( $name, 'aria-' ) === 0
				) {
		...
		$this->add_render_attribute( 'author_link', 'href', esc_url( $author['posts_url'] ) );
	}
}
...
if ( $print_link ) {
	...
	if ( ! empty( $settings['posts_url']['is_external'] ) ) {
		$this->add_render_attribute( 'button', 'target', '_blank' );
	}

	if ( ! empty( $settings['posts_url']['nofollow'] ) ) {
		$this->add_render_attribute( 'button', 'rel', 'nofollow' );
	}

	if ( ! empty( $settings['posts_url']['custom_attributes'] ) ) {
		...
		foreach ( $attributes as $attribute ) {
			...
			if (
				in_array( $name, [ 'title', 'class', 'id', 'rel', 'role' ], true ) ||
				strpos( $name, 'data-' ) === 0 ||
				strpos( $name, 'aria-' ) === 0
			) {
				$this->add_render_attribute(
...
<div class="xpro-author-box">
	...
	<div class="xpro-author-box-text">
		...
		<<?php echo esc_attr( $link_tag ); ?> <?php $this->print_render_attribute_string( 'author_link' ); ?>>
			<<?php echo esc_attr( $author_name_tag ); ?> class="xpro-author-box-name">
				<?php echo esc_html( $author['display_name'] ); ?>
			</<?php echo esc_attr( $author_name_tag ); ?>>
		</<?php echo esc_attr( $link_tag ); ?>>
		<?php endif; ?>
