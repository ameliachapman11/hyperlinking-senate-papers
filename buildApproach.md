...
The overall recommendation for the build structure of the prototype is to use Drupal for website development, Solr for the AHHH

Pursuing this prototype as a grounds-up project where we are doing the fullstack development ourselves rather than with the aid of technologies such as Content Management Systems allows for maximum freedom over design and functionality. It means that the prototype can completely align with University of Edinburgh branding guidelines and we have control over how search through the document is performed, which type of database we are using to store the papers, etc. However, this 
